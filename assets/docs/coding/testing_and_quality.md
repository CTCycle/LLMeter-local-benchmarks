...
The current local audit evidence covers 147 serialized all-target/all-feature tests on Windows at the 2026-09-26 validation boundary. Native hosted Linux/Windows workflow execution is defined in CI and release workflows but must still be treated separately from local evidence when it has not been run for the current commit.
