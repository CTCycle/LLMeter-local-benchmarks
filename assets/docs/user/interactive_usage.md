...
The model inventory keeps a client-local catalog cache for ordinary listing and metadata navigation. Use **Refresh exposed models** after loading or unloading a provider model; benchmark validation and measured/status paths independently refresh the provider catalog. Model identity comes from the provider `/v1/models` `id` field.
...
1. **Quick benchmark** - runs the canonical `smoke` performance profile and uses Markdown reporting by default.
...
3. **Standard LLM benchmark** - runs the guided LLM benchmark suite.
4. **Embeddings benchmark** - runs the guided embeddings suite.
...
Guided performance execution uses the same profile defaults as `llmeter bench perf`: `smoke` uses 3 measured runs, `latency` 5, `throughput` 4, and `sweep` 3; each profile defaults to one warmup request. Capability probing defaults to **skip probe** and telemetry defaults to off, matching the scriptable command defaults. The first-request load estimate remains enabled by default.
...
Capability probing under provider setup and performance benchmarking reports each endpoint check during validation when explicitly selected, so basic and full probes stay visible while `/v1/models`, chat, streaming, embeddings, or responses checks run.
