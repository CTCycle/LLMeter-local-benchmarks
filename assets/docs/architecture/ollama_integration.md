...
By default, `llmeter server stop` stops only that tracked process. This is safer than killing a server started by another terminal or by the user's operating system service manager. `--force` can be used for broader process termination.
