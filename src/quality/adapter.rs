...
pub fn build_quality_plan(framework: QualityFramework, task: &str, model: &str) -> QualityPlan {
    let command_preview = match framework {
        QualityFramework::LightEval => vec![format!(
            "uvx lighteval endpoint openai {task} --model-args model_name={model},base_url=http://localhost:8000/v1"
        )],
        QualityFramework::Inspect => vec![format!(
            "uvx inspect-ai eval {task} --model openai/{model} --model-base-url http://localhost:8000/v1"
        )],
        QualityFramework::LmEvalHarness => vec![format!(
            "uvx lm_eval --model local-completions --model_args model={model},base_url=http://localhost:8000/v1 --tasks {task}"
        )],
